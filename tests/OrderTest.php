...
class OrderTest extends TestCase
{
	...
	public function test_order_can_be_loaded_using_id_from_db()
	{
		...
		$this->assertSame('Roadster Men Jeans 001', $order->orderItems()[0]->product()->title);
		$this->assertSame('Roadster Men T-Shirt 001', $order->orderItems()[1]->product()->title);

		// Delete
		OrderItemClass::find($order->orderItems()[0]->thisId())->first()->delete();
		OrderItemClass::find($order->orderItems()[1]->thisId())->first()->delete();
		ProductClass::find($product_1->thisId())->first()->delete();
		ProductClass::find($product_2->thisId())->first()->delete();
		$loaded_order->delete();
	}
